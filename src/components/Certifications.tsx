...
export default function Certifications() {
  return (
    <section className="py-32 px-6 md:px-12 bg-matte-black overflow-hidden">
      <div className="max-w-7xl mx-auto">
            ...
            {CERTIFICATIONS.map((cert, index) => (
                ...
                >
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <h3 className="text-lg font-display font-bold text-soft-white leading-tight mb-1">{cert.title}</h3>
                            <span className="text-[10px] uppercase tracking-widest font-bold opacity-30">{cert.issuer}</span>
                        </div>
                    </div>
                    <a 
                        href={cert.link} 
        ...
        <div className="mt-32 p-12 bg-charcoal/30 border border-soft-white/10 rounded-sm text-center relative overflow-hidden group">
            <div className="absolute inset-0 bg-electric-blue/5 opacity-0 group-hover:opacity-100 transition-opacity" />
            <span className="text-xs uppercase tracking-[0.2em] font-bold text-soft-silver mb-4 block relative z-10">Beyond Development</span>
            <p className="text-2xl font-display font-medium text-soft-white/80 max-w-2xl mx-auto relative z-10">
                Solved <span className="text-white font-bold">170+ DSA problems</span> with a focus on optimized algorithm design and complex problem solving.
            </p>
        </div>
      </div>
    </section>
